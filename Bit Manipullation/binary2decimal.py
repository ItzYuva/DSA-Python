...
def binary2decimal(num:str)-> int:
    ...
    while idx>= 0:
        x = int(num[idx])*(2**power) # Convert the character to an integer and calculate its decimal value
        decimal_num += x # Add the decimal value to the total
        idx -= 1
        power += 1
    return decimal_num

num = "1001"
print(binary2decimal(num))
